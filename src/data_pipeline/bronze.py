...
from data_pipeline.schemas import SavMetaRecord
...
def parse_sav_meta(source_path: Path, source_filename: Path | str) -> Sequence[SavMetaRecord]:
    ...
    Returns
    -------
    A list of SavMetaRecord instances to be written to a database.
    """
    sav_metadata: list[SavMetaRecord] = []
...
def stream_to_bronze(
    ...
    # Read sample table for the schema
...
def read_table_meta_to_db(db_file: Path | str, source_path: Path, source_filename: Path | str) -> None:
    ...
    table.create_from_record(sav_metadata[0])
    table.insert_many(sav_metadata)
...
def ingest_source(
    ...
    # Update or create table metadata
    read_table_meta_to_db(source_manifest.db_file, source_path, source_filename)
    logger.info("Done.")
...
# variable_value_labels -> for categorical labels and missing numeric labels
# column_names_to_labels for descriptions

# schema: mapping of column name to pl type. uses meta.readstat_variable_types
# categorical labels: uses variable_value_labels - this is for generating the codebook (the categorical labels)
# missing numeric lables: variable_value_labels - this is for the conversion to silver later (coding them as NA)
# current code does not store them
# descriptions: from column_names_to_labels

# Goal: process as little as possible here? and then use a separate function that creates the codebook etc
# we need to store, for each raw file:
# column with categorical labels
# column with missing numeric labels --- check on real data; current code does not handle this correctly

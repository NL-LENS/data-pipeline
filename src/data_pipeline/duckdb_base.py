"""Base classes for interacting with tables and rows in a DuckDB database.

The idea is to express the schema of (relational) database in dataclasses,
and interact with the database through these classes. Along with
Python's type system, the classes enable to keep the code and the table
schema consistent.
...
**Class `DuckDBTable`**
- A dataclass for creating a table in a database,
reading data from it, and updating rows.

**Class `DuckDBRecord`**
- A dataclass containing data of one row in a table.
- The fields of the dataclass define the column names in the table;
the values of the fiels define the column values in that row.

Primary and foreign keys can be defined through the
metadata fields in the record class.

The two classes are closely linked, as illustrated by this workflow.

Intended workflow
-----------------
- Create a subclass of `DuckDBRecord`, defining the fields and types.
- Create a subclass of `DuckDBTable`, defining the name of the table, and create
one instance.
- Create multiple instances of records and one instance of the table.
- To create the table in the database, call the table's `create` method and pass
one record object.
- To write one or multiple records to the database table, pass the respective
records to the `insert` or `insert_many` method of the table class.
- To read a row from an existing database, create a record from the table.
- To update a row in an existing database, use the table's `update` method, by
...
class ForeignKey:
    ...
    def from_dict(cls, fk_dict: dict[str, dict[str, str]]) -> Self:
        ...
        ---------
        fk_dict:
            A dictionary where keys are column names (or field names),
            and values are dictionaries indicating the reference table
            and column that column refers to.
            The inner dictionaries should have keys "table" and "column".


        Examples
        --------
        When used directly, it works as follows:
        >>> fk_dict = {"a": {"table": "t3", "column": "id"}, "b": {"table": "t3", "column": "j"}}
        >>> ForeignKey.from_dict(fk_dict)

        This creates a composite foreign key on columns (a,b) referring to
        columns (id, j) in table t3.

        The more common usage is to declare a foreign
        key through a subclass of `DuckDBRecord`.
        >>> @dataclass
        >>> class TableMetaData(DuckDBRecord):
        >>>    path: str = field(metadata={"foreign_key": {"table": "t1", "column": "path"}})
        >>>    filename: str = field(metadata={"foreign_key": {"table": "t1", "column": "filename"}})
        >>>    schema: dict[str, str]
